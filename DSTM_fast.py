...
class DSTM_para(utils.SaveLoad):
    ...
    #初始化文档-主题比例参数gamma,维数M*n_max*K
        ...
        gamma_word =np.full(shape=[corpus_len,n_max,num_topics],fill_value=0,dtype=np.float)
        nkw = np.full(shape=[vocab_len,num_topics],fill_value=0,dtype=np.float)#对应双稀疏Nkr
        nkw_sum = np.full(shape=[num_topics],fill_value=0,dtype=np.float)#对应双稀疏Nk
        nmk = np.full(shape=[corpus_len,num_topics],fill_value=0,dtype=np.float)#对应双稀疏Njk
        nmk_sum = np.full(shape=[corpus_len],fill_value=0,dtype=np.float)#对应双稀疏Nm
    ...
    def gamma_update(self,gamma_word,nkw,nkw_sum,nmk,nmk_sum,amk,amk_sum,bkv,bkv_sum):
        ...
        for m in range(corpus_len):
            ...
            for n in range(n_per):
                ...
                word_freq = corpus[m][n][1]
                # 初始化临时参数
                gamma_word_temp = np.full(shape=[num_topics], fill_value=0, dtype=np.float)
